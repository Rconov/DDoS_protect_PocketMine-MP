...
class API_3_0_0 implements Listener {
    ...
    public function init(): void {
        $this->plugin->getLogger()->info("Инициализация защиты от DoS/DDoS для API 3.0.0");
        $this->initConfigs();
        if ($this->dosConfig->get("enable.guard", true)) {
            $this->plugin->getServer()->getPluginManager()->registerEvents($this, $this->plugin);

            // Используем PluginTask вместо анонимного класса для совместимости с API 3.0.0
            $this->plugin->getServer()->getScheduler()->scheduleRepeatingTask(new class($this->plugin, $this) extends PluginTask {
                private API_3_0_0 $api;

                public function __construct(PluginBase $plugin, API_3_0_0 $api) {
    ...
    private function handleDoS(DataPacketReceiveEvent $event, string $ip, $player): void {
        ...
        if ($this->packetCounter[$ip] > $this->dosConfig->get("max.packages", 100)) {
    ...
    private function handleDDoS(DataPacketReceiveEvent $event, string $ip, $player): void {
        ...
        if ($this->ddosIPs[$ip]["count"] > $this->ddosConfig->get("max.requests", 500)) {
            $this->banIP($ip, $this->ddosIPs[$ip]["count"], "DDoS");
            $event->setCancelled();
            $player->kick("Вы заблокированы за DDoS атаку.", false);
        }
        if ($elapsedTime > $this->ddosConfig->get("reset.time", 1)) {
    ...
    private function banIP(string $ip, int $rate, string $type): void {
        ...
        // Используем PluginTask для задержки снятия бана
        $this->plugin->getServer()->getScheduler()->scheduleDelayedTask(new class($this->plugin, $ip, $this) extends PluginTask {
            private string $ip;
            private API_3_0_0 $api;

            public function __construct(PluginBase $plugin, string $ip, API_3_0_0 $api) {
                parent::__construct($plugin);
                $this->ip = $ip;
                $this->api = $api;
            }
